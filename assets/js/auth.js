...
function verificaAcessoEspecial() {
    firebase.auth().onAuthStateChanged(user => {
        const uidUsuarioEspecifico = '3VgayE9I2NU4wA1y5R6r0cWIXjD2';
        if (user && user.uid === uidUsuarioEspecifico) {
            console.log('Usuário autenticado e autorizado.');
        } else {
            window.location.href = '/home.html';
            window.alert("⚠️Área restrita! Você será redirecionado para página principal!")
        }
    });
}  